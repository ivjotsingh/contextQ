...
class VectorStoreService:
    ...
    def __init__(self) -> None:
        ...
        # Voyage AI voyage-3-lite uses 1024 dimensions
    ...
    async def initialize(self) -> None:
        ...
        Creates collection with proper configuration for cosine similarity.
        """
    ...
    async def check_hash_exists(
        ...
        try:
            ...
            logger.warning("Error checking hash existence: %s", e)
            return None
    ...
    async def upsert_chunks(
        ...
        try:
            ...
            logger.info(
                "Upserted %d chunks for doc %s in %.2fs",
                len(points),
                doc_id,
                elapsed,
            )
    ...
    async def search(
        ...
        try:
            ...
            elapsed = time.time() - start_time
            logger.debug(
                "Search returned %d results in %.3fs",
                len(results.points),
                elapsed,
            )
    ...
    async def get_session_documents(
        ...
        try:
            # Scroll through all points for this session
            results = await self.client.scroll(
                collection_name=self.collection_name,
                scroll_filter=qdrant_models.Filter(
                    must=[
            ...
            logger.exception("Failed to get session documents")
