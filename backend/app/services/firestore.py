...
def _load_firebase_credentials(creds_value: str) -> dict:
    ...
    try:
        ...
        raise ValueError(
            f"FIREBASE_CREDENTIALS is neither valid JSON nor a valid file path"
        )
...
class FirestoreService:
    ...
    async def add_message(
        ...
        Args:
            ...
            sources: Optional source passages for assistant messages

        Returns:
            The document ID of the created message
        """
        try:
            message_data = {
                "role": role,
            ...
            # Add to session's messages subcollection
            doc_ref = (
    ...
    async def build_chat_context(
        ...
        If there are more than max_messages, uses summary + recent messages.
        ...
        try:
            ...
            if message_count <= max_messages:
                # Just use the messages directly
                return self._format_messages_for_context(messages)

            # Need to include summary
    ...
    async def update_session_activity(self, session_id: str, first_message: str | None = None) -> None:
        ...
        Args:
            session_id: Session identifier
            first_message: Optional first user message to generate title
        """
        try:
            session_ref = self.db.collection("sessions").document(session_id)
