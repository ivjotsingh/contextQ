...
class TestDocumentParser:
    ...
    def test_sanitize_filename_basic(self):
        ...
        result = self.parser.sanitize_filename("document.pdf")
        assert result == "document.pdf"

    def test_sanitize_filename_path_traversal(self):
        """Test path traversal prevention."""
        result = self.parser.sanitize_filename("../../../etc/passwd")
        assert "/" not in result
        assert ".." not in result

    def test_sanitize_filename_special_chars(self):
        """Test special character removal."""
        result = self.parser.sanitize_filename('doc<>:"/\\|?*.pdf')
        assert "<" not in result
        assert ">" not in result
        assert ":" not in result
    ...
    def test_sanitize_filename_long_name(self):
        ...
        long_name = "a" * 300 + ".pdf"
        result = self.parser.sanitize_filename(long_name)
    ...
    def test_sanitize_filename_empty(self):
        ...
        with pytest.raises(ValueError):
            self.parser.sanitize_filename("")

    def test_sanitize_filename_dot_start(self):
        """Test dot-starting filename handling."""
        result = self.parser.sanitize_filename(".hidden")
        assert not result.startswith(".")

    def test_validate_file_supported_types(self):
        """Test validation of supported file types."""
        # PDF
        ext = self.parser.validate_file("doc.pdf", 1000)
        assert ext == ".pdf"

        # DOCX
        ext = self.parser.validate_file("doc.docx", 1000)
        assert ext == ".docx"

        # TXT
        ext = self.parser.validate_file("doc.txt", 1000)
        assert ext == ".txt"

    def test_validate_file_unsupported_type(self):
        """Test rejection of unsupported file types."""
        with pytest.raises(UnsupportedFileTypeError):
            self.parser.validate_file("doc.exe", 1000)

        with pytest.raises(UnsupportedFileTypeError):
            self.parser.validate_file("doc.jpg", 1000)

    def test_validate_file_too_large(self):
        """Test rejection of files exceeding size limit."""
        large_size = 100 * 1024 * 1024  # 100MB
        with pytest.raises(FileTooLargeError):
            self.parser.validate_file("doc.pdf", large_size)
    ...
    def test_compute_content_hash(self):
        ...
        hash1 = self.parser.compute_content_hash(text)

        # Same text should produce same hash
        hash2 = self.parser.compute_content_hash(text)
        assert hash1 == hash2

        # Different text should produce different hash
        hash3 = self.parser.compute_content_hash("Different content")
        assert hash1 != hash3

    def test_compute_content_hash_normalized(self):
        """Test hash is normalized (case, whitespace)."""
        hash1 = self.parser.compute_content_hash("Hello World")
        hash2 = self.parser.compute_content_hash("hello   world")
    ...
    async def test_parse_txt_file(self):
        ...
        try:
            result = await self.parser.parse_file(temp_path, "test.txt")
    ...
    async def test_parse_empty_txt_file(self):
        ...
        try:
            with pytest.raises(EmptyDocumentError):
                await self.parser.parse_file(temp_path, "empty.txt")
        finally:
            os.unlink(temp_path)
    ...
    async def test_parse_whitespace_txt_file(self):
        ...
        try:
            with pytest.raises(EmptyDocumentError):
                await self.parser.parse_file(temp_path, "whitespace.txt")
        finally:
            os.unlink(temp_path)
    ...
    def test_normalize_text(self):
        ...
        result = self.parser._normalize_text(text)
