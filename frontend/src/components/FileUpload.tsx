...
export function FileUpload({ onUpload, isUploading, uploadProgress }: FileUploadProps) {
    ...
    const onDrop = useCallback(
        async (acceptedFiles: File[], rejectedFiles: any[]) => {
            ...
            const rejected: UploadStatus[] = rejectedFiles.map(({ file, errors }) => ({
                file: file.name,
                status: 'error',
            ...
            if (acceptedFiles.length > 0 && onUpload) {
                ...
                const statuses: UploadStatus[] = results.map(result => ({
                    file: result.file,
    ...
    const { getRootProps, getInputProps, isDragActive, isDragReject } = useDropzone({
        onDrop,
        accept: UPLOAD_CONSTANTS.ACCEPTED_TYPES,
        maxSize: UPLOAD_CONSTANTS.MAX_SIZE,
        multiple: true,
    });
    ...
    return (
        <div className="space-y-4">
            <div
                {...getRootProps()}
                className={`upload-zone ${isDragActive && !isDragReject ? 'upload-zone-active' : ''
            ...
            {uploadStatus.length > 0 && (
                <div className="space-y-2 animate-in">
                    {uploadStatus.map((item, index) => (
                        <div
                            key={`${item.file}-${index}`}
                            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm ${item.status === 'success'
